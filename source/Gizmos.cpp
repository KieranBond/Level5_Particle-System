...
void Gizmos::addBillboard(const glm::vec3 a_position, const glm::vec3 a_size, const glm::vec4 a_colour, const glm::mat4 a_cameraMat)
{
	glm::vec4 billColour = a_colour;
	//a_colour = glm::mix(glm::vec4(1.f, 0.f, 0.f, 1.f), glm::vec4(0.f, 1.f, 0.f, 1.f), 1.0f);

	glm::vec3 vX(a_size.x, 0, 0);
	...
	vertex[0] = a_position - nvX.xyz - vY;
	vertex[1] = a_position - nvX.xyz + vY;
	vertex[2] = a_position + nvX.xyz + vY;
	vertex[3] = a_position + nvX.xyz - vY;

	//Drawing the square
	addTri(vertex[0], vertex[3], vertex[2], billColour);
	addTri(vertex[0], vertex[2], vertex[1], billColour);
}
